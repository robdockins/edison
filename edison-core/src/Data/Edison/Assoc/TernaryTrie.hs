...
instance (Arbitrary k, Ord k, Arbitrary a) => Arbitrary (FM k a) where
  arbitrary = fromSeq <$> (arbitrary :: Gen [([k], a)])
  shrink xs = fromSeq <$> shrink (toSeq xs :: [([k], a)])
...
instance (Show k, Show a) => Show (InternalTest (FM k a)) where
  show (InternalTest (FM a m)) = "InternalTest (FM " ++ showsPrec 11 a "" ++ " " ++ showsPrec 11 m "" ++ ")"
...
instance (Integral k, Arbitrary k, Arbitrary a) => Arbitrary (InternalTest (FM k a)) where
  arbitrary = InternalTest <$> oneof [genFM, arbitrary]
  shrink (InternalTest (FM v m)) = [InternalTest (FM v m) | (v, FMB' m) <- shrinkTuple shrink shrinkFMB' (v, FMB' m)]
